...
const getHandlerElasticsearchFilter = (
  ...
  type: T,
): Option<QueryDslQueryContainer[]> => pipe(
...
const makeConditions = (
  dataset: Dataset,
  path: string[],
): ((conditions: { [k: string]: any }) => Option<QueryDslQueryContainer[]>) => flow(
  Object.entries,
...
const makeAndConditions = (
  dataset: Dataset,
): ((filterArgs: FilterArg[]) => Option<QueryDslQueryContainer[]>) => flow(
...
const makeFilter = (dataset: Dataset) => (filterArgs: FilterArg): Option<QueryDslQueryContainer> => {
...
const getHandlerElasticsearchSort = (
  ...
  type: T,
): Option<SearchSortCombinations[]> => pipe(
  selectHandler(type, path),
...
const makeIndividualSort = (
  dataset: Dataset,
  path: string[],
): ((order: { [k: string]: any }) => Option<SearchSortCombinations[]>) => flow(
  Object.entries,
  head,
...
const makeSort = (
  dataset: Dataset,
): ((order: { [k: string]: any }[]) => Option<NonEmptyArray<SearchSortCombinations>>) => flow(
...
function makeQueryBody(dataset: Dataset, args: DatasetArguments): NonNullable<SearchRequest['body']> {
  return {
    query: pipe(
      ...
      getOrElse((): QueryDslQueryContainer => ({ match_all: {} })),
    ),
    sort: pipe(
...
export const makeElasticsearchResolver = (
  performSearch: (dataset: Dataset, search: SearchRequest['body']) => Promise<SearchResponse>,
) => (
  dataset: Dataset,

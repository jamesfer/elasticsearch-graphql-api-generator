import {
  QueryContainer,
  SearchRequest,
  SearchResponse, SortCombinations,
...
describe('makeElasticsearchResolver', () => {
  ...
  describe('without custom fields', () => {
    let mockSearcher: jest.Mock<Promise<SearchResponse>, [Dataset, SearchRequest]>;
    let runRequest: (request: DatasetArguments) => Promise<any>;

    beforeEach(() => {
      mockSearcher = jest.fn<Promise<SearchResponse>, [Dataset, SearchRequest]>(async () => emptyResponse);
    ...
    describe('with filters', () => {
      it('adds a filter from a handler', async () => {
        ...
        expect(mockSearcher).toHaveBeenCalledWith<[Dataset, SearchRequest]>(baseDataset, {
          body: {
            query: {
              bool: {
                filter: [{
  ...
  describe('with custom fields', () => {
    ...
    describe('custom fields with filter support', () => {
      it('includes computed fields in elasticsearch filters', async () => {
        ...
        expect(mockSearcher).toHaveBeenCalledWith<[Dataset, SearchRequest]>(dataset, {
          body: {
            query: {
              bool: {
                filter: [{
      ...
      describe('when the computed field elasticsearch filter is a function', () => {
        let computedFilter: jest.Mock<QueryContainer[], [string[], any]>;
        let dataset: Dataset;

        beforeEach(async () => {
          computedFilter = jest.fn<QueryContainer[], [string[], any]>().mockReturnValue([{
            term: { x: 'hello' },
          }]);
          dataset = {
            ...baseDataset,
    ...
    describe('custom fields with order support', () => {
      ...
      describe('when the computed field elasticsearch sort is a function', () => {
        let computedSort: jest.Mock<SortCombinations[], [string[], any]>;
        let dataset: Dataset;

        beforeEach(async () => {
          computedSort = jest.fn<SortCombinations[], [string[], any]>().mockReturnValue(
            [{ name: 'desc' }],
          );
          dataset = {
            ...baseDataset,

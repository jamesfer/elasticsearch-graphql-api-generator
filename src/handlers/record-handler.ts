import { MappingProperty, QueryDslQueryContainer, SearchSortCombinations } from '@elastic/elasticsearch/api/types';
...
export const recordHandler: Handler<schema.RecordType, RecordFilterParameters, RecordOrderParameters> = {
  ...
  toElasticsearchMapping(record, path): MappingProperty {
    return {
      type: 'object',
      properties: generateRecordMappings(record, path),
    };
  },
  toElasticsearchFilter(record, path, parameters): QueryDslQueryContainer[] {
    return record.fields.flatMap(field => pipe(
      ...
      getOrElse<QueryDslQueryContainer[]>(() => []),
    ));
  },
  toElasticsearchSort(record, path, parameters): SearchSortCombinations[] {
    return record.fields.flatMap(field => pipe(
      fromNullable(parameters[field.name]),
      chain(getHandlerSort(field.type as schema.DefinedType, [...path, field.name])),
      getOrElse<SearchSortCombinations[]>(() => []),
    ));
  },
};

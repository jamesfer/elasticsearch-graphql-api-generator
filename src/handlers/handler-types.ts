import {
  MappingProperty, QueryDslQueryContainer, SearchSortCombinations,
...
export interface Handler<T extends schema.DefinedType, F = {}, S = {}, A = {}> {
   ...
   */
  toElasticsearchMapping?: MaybeNullableFunction<(field: T, path: string[]) => MappingProperty>;

  /**
   * Generates an Elasticsearch query fragment to add to the query based on the graphql
   * arguments provided in the query.
   */
  toElasticsearchFilter?: MaybeNullableFunction<(field: T, path: string[], parameters: F) => QueryDslQueryContainer[]>;

  /**
   * Generates an Elasticsearch sort fragment to add to the query based on the graphql
   * arguments provided in the query.
   */
  toElasticsearchSort?: MaybeNullableFunction<(field: T, path: string[], parameters: S) => SearchSortCombinations[]>;
}
...
export interface ComputedField<T = {}, A = {}, F = {}, S = {}> {
   ...
   * arguments provided in the query.
   */
  toElasticsearchFilter?: MaybeNullableFunction<(path: string[], parameters: F) => QueryDslQueryContainer[]>;

  /**
   * Generates an Elasticsearch sort fragment to add to the query based on the graphql
   * arguments provided in the query.
   */
  toElasticsearchSort?: MaybeNullableFunction<(path: string[], parameters: S) => SearchSortCombinations[]>;
}
